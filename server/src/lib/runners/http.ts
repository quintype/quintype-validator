...
const config = yaml.load(fs.readFileSync("config/rules.yml") as unknown as string) as HTTPValidationConfig;
...
function getContent($: CheerioSelector, element: CheerioElement, contentAttr: string): string {
...
async function runValidator(category: string, dom: CheerioSelector, url: string, response: RequestResponse): Promise<ValidationResult> {
  ...
  rules.forEach(rule => {
    switch (rule.type) {
      ...
      case 'dom': return validateDom(dom, rule as DomRule, url, outputLists);
      case 'url': return validateUrl(url, rule as UrlRule, outputLists);
      default: throw new Error(`Unknown rule type: ${rule.type}`);
    }
  });
  return { status: outputLists.errors.length === 0 ? "PASS" : "FAIL", ...outputLists };
}

export function runSeoValidator(dom: CheerioSelector, url: string, response: RequestResponse): Promise<ValidationResult> {
  return runValidator('seo', dom, url, response);
}

export function runHeaderValidator(dom: CheerioSelector, url: string, response: RequestResponse): Promise<ValidationResult> {
  return runValidator('headers', dom, url, response);
}

export function runOgTagValidator(dom: CheerioSelector, url: string, response: RequestResponse): Promise<ValidationResult> {
