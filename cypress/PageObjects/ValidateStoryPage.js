...
class ValidateStoryPage {
  selectStoryType() {
    ...
    cy.wait(1000);
    cy.get(`[class*="__menu"]`).contains("Story").click();
    return this;
  }
  ...
  verifyInvalidStoryCount(count) {
    const invalidStoryCount = cy.get(
      ":nth-child(3) > .migrator_statistic-reading__2H8Z4"
    );
  ...
  clickAccordian() {
    const arrowAccordian = cy.get(
      ".accordion-section_accordion-arrow__3AFqH > svg"
    );
  ...
  fileUpload() {
    ...
    cy.fixture(invalistoryslugfile, "binary")
      ...
      .then((fileContent) => {
        uploadButton.attachFile({
          fileContent,
          fileName: "invalistoryslugfile",
