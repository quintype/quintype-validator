...
const getErrorMessageFunction = (keyword: String): (error: ajv.ErrorObject, identifier: String, schema: String) => ErrorMessage => {
  switch (keyword) {
    ...
    case "type":
      return (error: ajv.ErrorObject, identifier: String, schema: String): ErrorMessage => {
        return { "id": identifier as string, "message": `${schema} with id ${identifier} has wrong type for ${error.dataPath}. It ${error.message}`, "logLevel": "error" }
      }
    case "required":
      return (error: ajv.ErrorObject, identifier: String, schema: String): ErrorMessage => {
        return { "id": identifier as string,"message": `${schema} with id ${identifier}  ${error.message}${error.dataPath !== '' ? ` in ${error.dataPath}` : ''}`, "logLevel": "error" }
      }
    case "anyOf":
      return (error: ajv.ErrorObject, identifier: String, schema: String): ErrorMessage => {
        return { "id": identifier as string, "message": `${schema} with id ${identifier} - has  wrong schema in ${error.dataPath}`, "logLevel": "error"}
      }
    case "enum": 
    return(error: ajv.ErrorObject, identifier: String, schema: String): ErrorMessage => {
      const params = error.params as  {allowedValues : ReadonlyArray<string>};
      return { "id": identifier as string,"message" : `${schema} with id ${identifier}  ${error.message} in ${error.dataPath} [ ${error.params && params.allowedValues.join(' ')} ]`, "logLevel": "error"}
    }
    default:
      return (error: ajv.ErrorObject, identifier: String, schema: String): ErrorMessage => {
        console.log(error,identifier,schema);
        return { "id": identifier as string, message: error.message || '', "logLevel": "warn" };
      }
  }
}
